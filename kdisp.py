...
def BackendReadProcess(micro_names,snaps,all_micros,kill_signal):
    ...
    if len(snaps)==len(all_micros):
        kill_signal.value=1
def BackendWriteProcess(micro_names,micro_dict,good_dir,bad_dir):
    for item in micro_names:
        if micro_dict[item] == 1:
            os.popen('ln -s ' + item + " " + good_dir)
        elif micro_dict[item] == -1:
            os.popen('ln -s ' + item + " " + bad_dir)
    pass
...
class DrawT(QThread):
    ...
    micro_name=""
   # thread_lock.lock()
    def __init__(self, windowid,micro_name):
        super(DrawT, self).__init__()
        self.micro_name=micro_name
    ...
    def run(self):
        ...
        try:
            ...
            item = QGraphicsPixmapItem(pix)
            self.scence.addItem(item)
            window_views[self.windowid]=self.scence
        except:
            pass
        #print(self.scence)
       # self.Qob.setScene(self.scence)
...
class Childwindow(QMainWindow,cw.Ui_childwindow):
    ...
    def save_selection(self):
        ...
        checked_groups=results[:current_checked]
        read = self.readtype
    ...
    def show_page(self):
        #print(self.Draw_Pool)
        '''
        temp_pool=self.Draw_Pool.copy()
        for drawthread in self.Draw_Pool:
            self.Draw_Pool[drawthread].quit()
            del temp_pool[drawthread]
        self.Draw_Pool=temp_pool.copy()
        temp_pool=None
        '''
        ...
        for result in self.group_results:
            ...
            self.Draw_Pool[resi] = DrawT(graphview, result)
            #self.Draw_Pool[resi]= DrawT(resi, result)
            self.Draw_Pool[resi].start()
            resi += 1
        '''
    ...
    def back_writer(self):
        #=======need 6 processes to write
        for pro_i in range(6):
            ...
            for window_i in range(self.groups):
                ...
                if current_position<self.reslen:
                    ...
                    #for writer , just add every filename in results[]
                    temp_group_results.append(results[current_position])

            backprocess=multiprocessing.Process(target=BackendWriteProcess,args=(temp_group_results,self.micro_dict,self.good_dir,self.bad_dir))
            backprocess.start()
...
class Mywindow(QMainWindow,mw.Ui_MainWindow):
    ...
    def __init__(self,parent=None):
        ...
        self.but_select.clicked.connect(self.act_select)
        self.but_pick.clicked.connect(self.act_pick)
        for gid in range (8):
            s=self.findChild(QCheckBox,"ck_gpu"+str(gid))
           # print(gid)
            s.clicked.connect(partial(self.check_slot,str(gid)))


    def act_pick(self):
       # self.main_text_append(self.current_work_directory)
      # self.main_text_append("")
       self.log_append("Curremt Gpus :"+self.current_gpu.toPlainText())
       self.main_text_append("Running Particle Picking job using Gautomatch...")
       cmd="repick_dir.sh "
      # os.popen()
       pass


    def main_text_append(self,message):
        current_text=self.main_text.toPlainText()
        out_text=current_text+str(message)+"\n"
        self.main_text.setText(out_text)
        self.main_text.moveCursor(self.main_text.textCursor().End)
        pass

    def act_select(self):
        results=QFileDialog.getExistingDirectory(self,"Choose Path",os.getcwd())
        #print(results)
        self.current_work_directory=results
        self.text_pwd.setText(self.current_work_directory)
        self.main_text_append("")
        self.log_append("Current Work Path: "+results)
        self.main_text_append("Running Select Micrographs Job...")
        self.main_text_append(cmd_exec("cd "+ results,0))
        cmd="pre.sh"
        self.main_text_append(cmd_exec(cmd,0))
        pass

    def log_append(self,message):
        current_date = cmd_exec("date", 1)
        self.main_text_append("==================================")
        self.main_text_append(current_date)
        self.main_text_append("-----------------")
        self.main_text_append(message)

    def check_slot(self, gid):
        #print(gid)
        try:
            if self.gpus.index(gid)!=-1:
                self.gpus.remove(gid)
            else:
                self.gpus.append(gid)
        except:
            self.gpus.append(gid)
        message=",".join(self.gpus)
        self.current_gpu.setText(message)
        pass


    def print_test(self):
        res=self.cmd_exec('top c',1)
        return res
